...
ミニマップはマスクから直接描く。主人公を隠す建物・塀・配置物は実行時にフェードで抜く（遮蔽の式は配置の助言）。F05 旧鹿之尾街道 商店街に続き、A 群（F01 / F06 / F12 / F13 / F02）は `docs/field/*_spec.md` と `refs/field/*.png` の座標から `tools/gen_fields_a.py` で生成する（フェーズ 12）。
