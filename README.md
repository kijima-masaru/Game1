...
ミニマップはマスクから直接描く。建物の高さは遮蔽の式（必要距離 = (高さ − 1.7) / tan 30°）で自動的に決まる。F05 旧鹿之尾街道 商店街に続き、A 群（F01 / F06 / F12 / F13 / F02）は `docs/field/*_spec.md` と `refs/field/*.png` の座標から `tools/gen_fields_a.py` で生成する（フェーズ 12）。
