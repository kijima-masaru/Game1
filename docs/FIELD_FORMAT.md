...
**近景の帯（フェーズ 12 Q-1）**: 歩ける範囲の西・南（カメラ側）に 2 タイル、東・北に 1 タイルの帯（128）。1 列目は歩道、2 列目に生垣・ブロック塀・金網・ガードレール（`barriers`）、
配置物は歩道と正面の前に寄せる。建物は両側に置き、手前側は遮蔽の式で平屋に落ち、奥側は 2 階建て。近景（5 m 以内）に面積の大きい平らな屋根と広い空き地を置かない。
計測は `areamask=1` の画像から: 屋根 10〜30 %、正面 15 % 以上、下端 1/3 の空白率（地面 + フィールド外）30 % 以下。
`tools/gen_fields_a.py` は道（walk）を置くだけで帯・塀・配置物を規則から自動で付ける。
