...
| 192 | `narrow` | 歩ける裏路地・袋小路。幅の制限なし。**調べ物・NPC・出入口は置けない**（検証エラー）。遮蔽の式の対象外（隠れてよい） |
...
| `floors` / `height_m` | 階数か軒高（m）。平屋 = 軒 2.3 + 棟 0.7 = 3.0 m、2 階建て = 軒 5.3 + 棟 1.4 = 6.7 m。**遮蔽の式（第 6 節）に違反すると自動で 1 階ずつ落ちる** |
...
## 6. 遮蔽の式（N-1）、近景の帯（P-2a）と検証

**近景の帯（フェーズ 11 P-2a）**: カメラは西南西（field_yaw 45〜50）なので、歩ける範囲の**西に 11 タイル、南に 4 タイル**は屋根を置かない帯（128）にする。
帯の 1 列目は歩道、2 列目に生垣・ブロック塀・ガードレール（`barriers`）、3 列目以降は草地と配置物。建物は帯の外か、歩ける範囲の**東・北**（正面がカメラに向く側）に
1 タイルの帯を挟んで置く。配置器は「歩けるタイルからカメラ方向 5 m 以内の建物」を検査して報告する。画面下端 1/3 の屋根の面積比（目標 15 % 以下）は
`areamask=1` の画像から測る。`tools/gen_fields_a.py` は道（walk）を置くだけで帯・塀・配置物を規則から自動で付ける。


- 歩けるタイル（`walk`）の主人公の頭（1.7 m）からカメラの方へ、水平 1 m につき tan(俯角 30°) 上がる視線が建物に当たれば違反。
  **必要距離 = (高さ − 1.7) / tan 30°**: 平屋の軒 2.3 m → 1.0 m、棟 3.0 m → 2.3 m、2 階建ての軒 5.3 m → 6.2 m、棟 6.7 m → 8.7 m。
  軒は壁の矩形まで、棟は正面と平行な中心線までの距離で判定する。違反する建物は階数を落とし、平屋でも違反なら図（`cache/fields/<ID>/occlusion.png`）に出す。
  通り抜ける構造物（山門）と裏路地は対象外。塀・生垣はそのまま。空白にはしない（外周は必ず建物か塀）。
