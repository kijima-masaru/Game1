...
- 「カメラ手前」はフィールドのローカルで `(-sin field_yaw, cos field_yaw)` の向き（タイル）。近景の帯（第 6 節）はこれで決まる。

## 1. 歩行可能マスク `<ID>_walkable.png`

大きさは `size` と同じ（F05 なら 32×40 px）。グレースケールで描く（RGB でも輝度で読む。アルファは掛け算）。

| 明るさ | 種別 | 意味 |
| --- | --- | --- |
| 白（≥ 218） | `walk` | 歩ける（主要）。**道幅は 6 タイル以上**（検証で縦横どちらかの連続長が 6 未満だとエラー） |
| 明るい灰（153〜217） | `narrow` | 歩ける裏路地・袋小路。幅の制限なし。**調べ物・NPC・出入口は置けない**（検証エラー） |
| 暗い灰（77〜152） | `open` | 歩けない空き地（駐車場・水面・植込み）。建物は置かない。`ground.open` のテクスチャで見せる |
| 黒（< 77） | `blocked` | 歩けない。歩ける範囲に接するタイルが建物の候補地になる |
...
```json
{
  ...
  "ground":   { "walk": "old_street", "narrow": "alley", "open": "gravel", "default": "lot_ground",
                "patches": [ { "id": "temple_yard", "rect": [18, 5, 13, 15], "tex": "gravel" } ] },
  "edge_fill": [ { "rect": [17, 4, 15, 17], "kind": "fence_wall", "reserve": true },
...
- `sun_az`: 時間帯ごとの太陽方位（度、カメラ基準。lookdev の `TIMES` と同じ定義）。**field_yaw とは独立。** 無い時間帯はプリセット値。
  F05 は夕方 230 で通りに西日が入る（`sun_az=` 引数で試して決める）。
...
```json
{ "id": "dagashi",   "kind": "dagashi",      "width": 3, "floors": 2, "wall": "weatherboard", "near": [5, 20] },
{ "id": "temple_hall", "kind": "temple_hall", "width": 10, "depth": 5, "height_m": 4.5, "wall": "plaster", "near": [24, 4] },
{ "id": "temple_gate", "kind": "temple_gate", "width": 3, "depth": 1, "front": "S", "at": [22, 20] },
{ "kind": "shop_wood", "width": 4, "floors": 2, "wall": "mortar" }
```

| キー | 意味 |
| --- | --- |
| `kind` | `shop_shutter` / `shop_wood` / `dagashi` / `house` / `temple_hall` / `temple_gate` / `bldg_rc` / 塀 3 種 |
| `width` | 間口（タイル）。既定 3 |
| `depth` | 奥行きの上限（タイル）。既定 3。裏に別の道や場外があれば自動で縮む。**2 未満しか取れない場所には置かない**（塀になる） |
| `floors` / `height_m` | 階数（1 階 3 m）か軒高（m）。**近景の帯では自動で平屋（軒 2.3 m + 棟 0.7 m = 3 m）に落ちる** |
...
- 歩けるタイルからカメラの方へ 5.2 m（4.55 タイル）、幅 ±0.75 タイルの視線の帯に入る黒タイルを「近景の帯」とする。
  帯に掛かる建物は平屋（3 m）に落とす。塀・生垣はそのまま。空白にはしない（外周は必ず建物か塀）。
...
- 主人公が隠れたときだけ手前の建物を透過する（判定は頭の高さ 1.6 m と実際の面）。3 m の建物が頭を隠すのは 2.4 m 以内。
