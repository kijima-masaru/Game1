...
| `data/fields/<ID>_height.png` | **任意。** 地面の高さ（1 px = 1 タイル、1 階調 = 0.15 m）。無ければ全面 0 m（第 8 節、フェーズ 14 T-2a/b） |
...
## 8. 高さ `<ID>_height.png`（フェーズ 14 T-2a 提案、T-2b 実装）
...
| 隣との差 | 扱い |
| --- | --- |
| 0〜1 階調（0〜0.15 m） | **登れる**（縁石・一段）。見た目は垂直の小さな面。承認時に 2 階調から 1 階調に厳しくした（またぐ動きは絵が軽くなる） |
| 2 階調以上（0.30 m〜） | **登れない**（擁壁・石垣・崖）。ただしどちらかのタイルが `slope` / `stairs` の矩形の中なら渡れる |
...
| `stairs` 階段 | 坂と同じ（動きは滑らか、段ごとに止まらない） | 1 タイルに（差の階調数）段の蹴上げ 0.15 m を刻む。踏面 = 1.143 / 段数。踏面は `tex`（既定 `stone_step`）、蹴上げは `wall`（既定 `stone_wall`） | **標準は +4 / タイル**（踏面 0.29 m、傾き 28°）。緩い石段 +2〜3、急な石段 +5〜6。矩形は一方向に単調（検証） |
| `bridge` 橋 | 8b の規則（橋の上は平ら） | 床板の厚み 0.4 m と欄干 1.0 m、6 タイルごとの橋脚。下の地面（川・道）まで壁で埋めない。`deck` で床板のテクスチャ | 橋のタイルを桁の高さにする。下を通る道は通れない（高さの二重は持たない） |
| `wall` 壁 | 8b の規則 | 何も変えない。矩形の中のタイルに接する段差の面のテクスチャだけ `wall` で差し替える（石垣・岩・土） | 値をそのまま |

坂・階段の向きは矩形の中の高さの勾配から求める。矩形は歩けるタイルの外（法面）にも置ける。矩形が重なったら 階段 > 坂 > 橋 > 壁 の順に優先し、同じ種別なら先に書いたものが勝つ。

縁の高さ（T-2b で決めた規則）:

- **坂**の 4 隅は「周りの 4 タイルのうち平らなもの（坂・階段でないもの）の平均。無ければ 4 タイルの中心の平均」。平らなタイルと接する縁はそのタイルの高さにぴたりと合う。
- **階段**の縁は**進行方向の隣だけ**で決める（隣が同じ階段なら中心の平均、外なら隣の高さ）。幅方向は一定なので、脇のタイルに引きずられない。
  脇のタイルは同じ勾配の段々（F08 の石段の両脇）にしても、平らな壁（F16 の岩壁）にしてもよい。
- 坂・階段の縁が隣の平らなタイルと合わないところには、自動で**裾**（垂直の面、`wall` のテクスチャ）が立つ。
- 坂・階段の中の足元は 4 隅の双線形補間。階段の踏面は補間面の ±0.075 m に収まる。
...
- **フェード**: 頭・腰の線分は足元の高さから。**3 階調以上の段差の面は建物と同じ α 0.35 のフェードの対象**（F03 の隧道で、高架の台の壁が主人公を隠したので決めた。T-2b）。2 階調以下の面は対象外。
  `walk_demo=1` の `terrain_occl_frames` は地形の面が視線に入ったフレーム数（診断）。
- **AO**: 地形の面も UV2 付きの板として焼く（生成物はキャッシュ、キーに高さ画像のハッシュを足す。テクスチャ版 5）。3 階調以上の段差の面は AO の遮蔽物にもなる（薄い箱）。
- **外周の帯**: 縁のタイルの高さで 16 タイル延ばし、帯どうしの段差は面で塞ぐ。
- **ミニマップ**: 2 階調以上の段差の境を暗い線で描く（歩ける・空き地のタイルに接するものだけ）。階段は矩形を細い横線で描く。高さそのものは描かない。
...
3. 石段は「段数 ÷ 4」タイルの長さで描く（80 段なら 20 タイル）。長さが地図で足りない場合は矩形を短くせず、段数を減らして `docs/field/DEVIATIONS.md` に記す。
4. 生成器 `tools/fieldgen.py`（`F` クラス）: `height(rect, v)`、`ramp(rect, axis, v0, v1)`（坂）、`stairs(...)`、`bridge(...)`、`wallzone(rect, wall)`。`tools/gen_fields_b.py` が 10 枚の例。
